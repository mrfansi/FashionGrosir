...
class Konfirmasi extends MY_Controller
{
    ...
    {
        $o_kode = $this->order
            ->where('o_noorder', $this->uri->segment(2))
            ->get()->o_kode;
        ...
        if ($order_bukti) {
            ...
            $konfirmasi = $this->order_bukti->where('o_kode', $o_kode)->update(array(
            ...
            if ($konfirmasi) {
                $this->order->where('o_kode', $o_kode)->update(array('o_status' => 3));
            }
        } else {
            //upload an image options
            $config = array();
    ...
    public function sukses()
    {
        ...
        $this->data->orders = $this->order->with_order_detil()->where_o_noorder($this->data->nomor_order)->get();
        $this->data->orders_total = function () {
            $hasil = 0;
            foreach ($this->data->orders->order_detil as $order) {
                $hasil += $order->od_tharga;
        ...
        $order = $this->order->where('o_noorder', $this->uri->segment(2))->get();
        if ($order->o_status == 3) {
